...
\t\t\t\t\t<div class=\"col-md-6\">
					<?php
						...
						echo form_dropdown('".$v['nama_form']."',\$dataTes,2,'id=\"".$v['nama_form']."\" class=\"form-control select\" style=\"width: 100%\"');
					?>
					</div>
				</div>";
}
		...
		if($('input[name=\"submit\"]').val()=='Save'){
			...
			$.ajax({
				...
				'success': function(html){
					...
					{
						infoDialog.realize();
						infoDialog.setTitle('Info').setMessage('Gagal !!').setType(BootstrapDialog.TYPE_DANGER).open();
					}					 
				},
				'error': function(){ 
					infoDialog.realize();
