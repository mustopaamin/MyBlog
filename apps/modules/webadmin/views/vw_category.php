...
<section class="content">
	<div class="box box-info"> <!-- start .box -->
		...
		<?php if($this->rolemodule->check_button('add')) { ?>			
			<a class="btn btn-sm btn-primary" id="addCategory"><i class="fa fa-plus"></i> Tambah</a>&nbsp;&nbsp;
		<?php } ?>
		</div> <!-- end .box-header -->
		
		<div class="box-body"> <!-- start .box-body -->
			<table id="tableCategory" class="table datares">
				<thead>
					<tr>				
						<th>No</th>				
						<th>Name</th>				
...
$(document).ready(function(){
	oTable = $('#tableCategory').dataTable({
		"processing": true,
		"serverSide": true,
		"sAjaxSource": "<?php echo site_url('webadmin/category/fnCategoryDataJson');?>",
		"sServerMethod": "POST",
		"aoColumns": [ 
			null,null,null,null ,{ 'bSortable': false,'sClass':'center' }
		],
	...
	$('#addCategory').click(function()	{
		window.location.href = "<?php echo site_url('webadmin/category/fnCategoryAdd');?>";
...
{
	window.location.href = "<?php echo site_url('webadmin/category/fnCategoryEdit');?>/"+idRow;
}

function fnDelete(idRow)
{
	BootstrapDialog.confirm('Yakin ingin dihapus ?', function(result){
		var urlUpdate='<?php echo site_url('webadmin/category/fnCategoryDelete');?>'
		if(result)
		{
