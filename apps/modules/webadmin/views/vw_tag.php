...
<section class="content">
	<div class="box box-info"> <!-- start .box -->
		...
		<?php if($this->rolemodule->check_button('add')) { ?>			
			<a class="btn btn-sm btn-primary" id="addTag"><i class="fa fa-plus"></i> Tambah</a>&nbsp;&nbsp;
		<?php } ?>
		</div> <!-- end .box-header -->
		
		<div class="box-body"> <!-- start .box-body -->
			<table id="tableTag" class="table datares">
				<thead>
					<tr>				
						<th>No</th>				
						<th>Name</th>				
...
$(document).ready(function(){
	oTable = $('#tableTag').dataTable({
		"processing": true,
		"serverSide": true,
		"sAjaxSource": "<?php echo site_url('webadmin/tag/fnTagDataJson');?>",
		"sServerMethod": "POST",
		"aoColumns": [ 
			null,null,null ,{ 'bSortable': false,'sClass':'center' }
		],
	...
	$('#addTag').click(function()	{
		window.location.href = "<?php echo site_url('webadmin/tag/fnTagAdd');?>";
...
{
	window.location.href = "<?php echo site_url('webadmin/tag/fnTagEdit');?>/"+idRow;
}

function fnDelete(idRow)
{
	BootstrapDialog.confirm('Yakin ingin dihapus ?', function(result){
		var urlUpdate='<?php echo site_url('webadmin/tag/fnTagDelete');?>'
		if(result)
		{
