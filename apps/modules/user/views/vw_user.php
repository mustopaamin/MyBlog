...
<section class="content">
	<div class="box box-info"> <!-- start .box -->
		...
		<?php if($this->rolemodule->check_button('add')) { ?>			
			<a class="btn btn-sm btn-primary" id="addUser"><i class="fa fa-plus"></i> Tambah</a>&nbsp;&nbsp;
		<?php } ?>
		</div> <!-- end .box-header -->
		
		<div class="box-body"> <!-- start .box-body -->
			<table id="tableUser" class="table datares">
				<thead>
					<tr>				
						<th>No</th>				
						<th>Email</th>				
...
$(document).ready(function(){
	oTable = $('#tableUser').dataTable({
		"processing": true,
		"serverSide": true,
		"sAjaxSource": "<?php echo site_url('user/fnUserDataJson');?>",
		"sServerMethod": "POST",
		"aoColumns": [ 
	...
	$('#addUser').click(function()	{
		window.location.href = "<?php echo site_url('user/fnUserAdd');?>";
...
{
	window.location.href = "<?php echo site_url('user/fnUserEdit');?>/"+idRow;
}

function fnDelete(idRow)
{
	BootstrapDialog.confirm('Yakin ingin dihapus ?', function(result){
		var urlUpdate='<?php echo site_url('user/fnUserDelete');?>'
		if(result)
		{
